...
namespace DialogueEditor
{
    public class ConversationManager : MonoBehaviour
    {
        ...
        private NPCConversation m_rawConversationAsset;
        ...
        private void Update()
        {
            switch (m_state)
            {
                case eState.TransitioningDialogueBoxOn: TransitioningDialogueBoxOn_Update(); break;
                case eState.ScrollingText: ScrollingText_Update(); break;
                case eState.TransitioningOptionsOn: TransitionOptionsOn_Update(); break;
                case eState.Idle: Idle_Update(); break;
                case eState.TransitioningOptionsOff: TransitionOptionsOff_Update(); break;
                case eState.TransitioningDialogueOff: TransitioningDialogueBoxOff_Update(); break;
            }
        }

        public void StartConversation(NPCConversation conversation)
        {
             m_rawConversationAsset = conversation;

            m_conversation = conversation.Deserialize();
            if (OnConversationStarted != null) OnConversationStarted.Invoke();
            TurnOnUI();
        ...
        public void SelectNextOption()
        {
            ...
            if (next > m_uiOptions.Count - 1) next = 0;
        ...
        public void SelectPreviousOption()
        {
            ...
            if (previous < 0) previous = m_uiOptions.Count - 1;
        ...
        {
            if (m_state != eState.Idle) { return; }
            if (m_currentSelectedIndex < 0 || m_currentSelectedIndex >= m_uiOptions.Count || m_uiOptions.Count == 0) { return; }
        ...
        public void SetInt(string paramName, int value)
        {
            ...
            if (status == eParamStatus.NoParamFound) LogWarning("parameter \'" + paramName + "\' does not exist.");
        }
        ...
        private void SetState(eState newState)
        {
            switch (m_state)
            {
                case eState.TransitioningOptionsOff: m_selectedOption = null; break;
            ...
            switch (m_state)
            {
                case eState.TransitioningDialogueBoxOn:
                    ...
                    break;
                case eState.ScrollingText: SetColorAlpha(DialogueText, 1); break;
                case eState.TransitioningOptionsOn:
                    SetColorAlpha(DialogueText, 1);
                    CreateUIOptions();
                    for (int i = 0; i < m_uiOptions.Count; i++) m_uiOptions[i].gameObject.SetActive(true);
                    break;
            }
        }
        ...
        private void TransitioningDialogueBoxOn_Update()
        {
            ...
            if (t > 1) { SetupSpeech(m_currentSpeech); return; }
        ...
        private void ScrollingText_Update()
        {
            ...
            float timePerChar = (60.0f / charactersPerSecond) * ScrollSpeed;
            ...
            if (m_elapsedScrollTime > timePerChar)
            {
                ...
                m_scrollIndex++;
                if (m_scrollIndex >= m_targetScrollTextCount) SetState(eState.TransitioningOptionsOn);
            }
        }
        ...
        private void TransitionOptionsOn_Update()
        {
            m_stateTime += Time.deltaTime;
            float t = m_stateTime / TRANSITION_TIME;
            if (t > 1) { SetState(eState.Idle); return; }
            for (int i = 0; i < m_uiOptions.Count; i++) m_uiOptions[i].SetAlpha(t);
        }
        ...
        private void Idle_Update()
        {
            ...
            if (m_currentSpeech.AutomaticallyAdvance)
            {
                ...
                {
                    if (m_stateTime > m_currentSpeech.TimeUntilAdvance) SetState(eState.TransitioningOptionsOff);
                }
            }
        }
        ...
        private void TransitionOptionsOff_Update()
        {
            ...
            {
                ClearOptions();
                if (m_currentSpeech.AutomaticallyAdvance && IsAutoAdvance()) return;
                if (m_selectedOption == null) { EndConversation(); return; }
                SpeechNode nextSpeech = GetValidSpeechOfNode(m_selectedOption);
                if (nextSpeech == null) EndConversation();
                else SetupSpeech(nextSpeech);
                return;
            }
            for (int i = 0; i < m_uiOptions.Count; i++) m_uiOptions[i].SetAlpha(1 - t);
        ...
        {
            if (speech == null) { EndConversation(); return; }
            ...
            else if (m_rawConversationAsset != null && m_rawConversationAsset.DefaultSprite != null)
            {
                NpcIcon.sprite = m_rawConversationAsset.DefaultSprite;
            }
            else
            {
                NpcIcon.sprite = BlankSprite;
            ...
            if (!string.IsNullOrEmpty(speech.Name))
            {
                NameText.text = speech.Name;
            }
            else if (m_rawConversationAsset != null && !string.IsNullOrEmpty(m_rawConversationAsset.DefaultName))
            {
                NameText.text = m_rawConversationAsset.DefaultName;
            }
            else
            {
                NameText.text = "";
            }
            DialogueText.font = speech.TMPFont != null ? speech.TMPFont : null;

            if (string.IsNullOrEmpty(speech.Text)) { DialogueText.text = ""; m_targetScrollTextCount = 0; }
            else { DialogueText.text = speech.Text; m_targetScrollTextCount = speech.Text.Length + 1; }

            if (speech.Event != null) speech.Event.Invoke();
            DoParamAction(speech);
            if (speech.Audio != null) { AudioPlayer.clip = speech.Audio; AudioPlayer.volume = speech.Volume; AudioPlayer.Play(); }
            if (ScrollText) SetState(eState.ScrollingText);
            else SetState(eState.TransitioningOptionsOn);
        }

        public void SpeechSelected(SpeechNode speech) { SetupSpeech(speech); }
        public void OptionSelected(OptionNode option)
        {
            m_selectedOption = option;
            DoParamAction(option);
            if (option.Event != null) option.Event.Invoke();
        ...
        private bool IsAutoAdvance()
        {
            ...
            {
                SpeechNode next = GetValidSpeechOfNode(m_currentSpeech);
                if (next != null) { SetupSpeech(next); return true; }
            }
            else if (m_currentSpeech.ConnectionType == Connection.eConnectionType.None) { EndConversation(); return true; }
            return false;
        }
        ...
        private SpeechNode GetValidSpeechOfNode(ConversationNode parentNode)
        {
            ...
            for (int i = 0; i < parentNode.Connections.Count; i++)
            {
                ...
                if (ConditionsMet(connection)) return connection.SpeechNode;
            }
            return null;
        }
        ...
        private void TurnOnUI()
        {
            ...
            if (BackgroundImage != null) { DialogueBackground.sprite = BackgroundImage; DialogueBackground.type = BackgroundImageSliced ? Image.Type.Sliced : Image.Type.Simple; }
            NpcIcon.sprite = BlankSprite;
        }

        private void TurnOffUI() { DialoguePanel.gameObject.SetActive(false); OptionsPanel.gameObject.SetActive(false); SetState(eState.Off); }
        ...
        private void CreateUIOptions()
        {
            if (m_currentSpeech.ConnectionType == Connection.eConnectionType.Option)
            {
                for (int i = 0; i < m_currentSpeech.Connections.Count; i++)
                {
                    ...
                    if (ConditionsMet(connection)) { UIConversationButton uiOption = CreateButton(); uiOption.SetupButton(UIConversationButton.eButtonType.Option, connection.OptionNode); }
                }
            }
            else
            {
                ...
                if (notAutoAdvance || allowVisibleOptionWithAuto)
                {
                    if (m_currentSpeech.ConnectionType == Connection.eConnectionType.Speech)
                    {
                        // MOD: Không sinh nút Speech (Continue) nếu bạn muốn dùng Click Background
                        // Nếu vẫn muốn nút "Continue" hiện ra thì để nguyên, nếu muốn xóa nút thì comment đoạn dưới.
                        /*
                        UIConversationButton uiOption = CreateButton();
                        SpeechNode next = GetValidSpeechOfNode(m_currentSpeech);
                        if (next == null) uiOption.SetupButton(UIConversationButton.eButtonType.End, null, endFont: m_conversation.EndConversationFont);
                        else uiOption.SetupButton(UIConversationButton.eButtonType.Speech, next, continueFont: m_conversation.ContinueFont);
                        */
                    }
                    else if (m_currentSpeech.ConnectionType == Connection.eConnectionType.None)
                    {
                        // MOD: XÓA VIỆC SINH NÚT "END."
                    }
                }
            }
            
            // MOD: THAY VÌ CHỌN NÚT ĐẦU TIÊN (SetSelectedOption(0)), TA SẼ HỦY CHỌN
            UnselectOption();

            for (int i = 0; i < m_uiOptions.Count; i++) { m_uiOptions[i].SetImage(OptionImage, OptionImageSliced); m_uiOptions[i].SetAlpha(0); m_uiOptions[i].gameObject.SetActive(false); }
        }

        private void ClearOptions() { while (m_uiOptions.Count != 0) { GameObject.Destroy(m_uiOptions[0].gameObject); m_uiOptions.RemoveAt(0); } }

        private void SetColorAlpha(MaskableGraphic graphic, float a) { Color col = graphic.color; col.a = a; graphic.color = col; }

        private void SetSelectedOption(int index)
        {
            if (m_uiOptions.Count == 0) { return; }
            if (index < 0) index = 0;
            if (index > m_uiOptions.Count - 1) index = m_uiOptions.Count - 1;
            if (m_currentSelectedIndex >= 0 && m_currentSelectedIndex < m_uiOptions.Count) m_uiOptions[m_currentSelectedIndex].SetHovering(false);
        ...
        private UIConversationButton CreateButton() { UIConversationButton button = GameObject.Instantiate(ButtonPrefab, OptionsPanel); m_uiOptions.Add(button); return button; }
        ...
        private bool ConditionsMet(Connection connection)
        {
            ...
            for (int i = 0; i < conditions.Count; i++)
            {
                ...
                if (conditions[i].ConditionType == Condition.eConditionType.IntCondition)
                {
                    ...
                    eParamStatus status; int currentValue = m_conversation.GetInt(condition.ParameterName, out status);
                    if (condition.CheckType == IntCondition.eCheckType.equal) conditionMet = (currentValue == condition.RequiredValue);
                    else if (condition.CheckType == IntCondition.eCheckType.greaterThan) conditionMet = (currentValue > condition.RequiredValue);
                    else if (condition.CheckType == IntCondition.eCheckType.lessThan) conditionMet = (currentValue < condition.RequiredValue);
                }
                if (conditions[i].ConditionType == Condition.eConditionType.BoolCondition)
                {
                    ...
                    eParamStatus status; bool currentValue = m_conversation.GetBool(condition.ParameterName, out status);
                    if (condition.CheckType == BoolCondition.eCheckType.equal) conditionMet = (currentValue == condition.RequiredValue);
                    else if (condition.CheckType == BoolCondition.eCheckType.notEqual) conditionMet = (currentValue != condition.RequiredValue);
                }
        ...
        public void DoParamAction(ConversationNode node)
        {
            ...
            for (int i = 0; i < node.ParamActions.Count; i++)
            {
                ...
                if (node.ParamActions[i].ParamActionType == SetParamAction.eParamActionType.Int) SetInt(name, (node.ParamActions[i] as SetIntParamAction).Value);
                else if (node.ParamActions[i].ParamActionType == SetParamAction.eParamActionType.Bool) SetBool(name, (node.ParamActions[i] as SetBoolParamAction).Value);
            }
        }
